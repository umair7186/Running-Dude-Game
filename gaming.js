...
var game = new Phaser.Game(800, 320, Phaser.AUTO, 'screen', { preload: preload, create: create, update: update });

function preload() {
    game.stage.backgroundColor= '#F0E68C';
    game.load.tilemap('collect', 'assets/stg1.json', null, Phaser.Tilemap.TILED_JSON);
...
function create() {
    ...
    map.setCollisionBetween(901, 914);
    map.setCollisionBetween(967, 970);
    map.setCollisionBetween(951, 964);
    map.setCollisionBetween(945, 948);
...
//   layer.debug = true;
    ...
    scoreText = game.add.text(590, 16, 'score: 0', { fontSize: '32px', fill: '#000' });
...
function update() {
     ...
     if(player.y>game.height)
    {
        ...
        fire.kill();
        enemy.kill();
         scoreText = game.add.text(400, 180, 'Game Over', { fontSize: '64px', fill: '#000' , align: 'center'});
         scoreText.fixedToCamera = true;
